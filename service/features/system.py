...
def detect_capabilities() -> dict:
    ...
    if vpc:
        if Path(f"{vpc}/fn_lock").exists():
            res["fn_lock"] = True
        if Path(f"{vpc}/flip_to_start").exists():
            res["flip_to_start"] = True
        if Path(f"{vpc}/instant_boot").exists():
            res["instant_boot"] = True
...
def set_flip_to_start(enabled: bool) -> None:
    vpc = get_vpc_path()
    if vpc and Path(f"{vpc}/flip_to_start").exists():
        Path(f"{vpc}/flip_to_start").write_text("1" if enabled else "0")
...
def set_instant_boot(enabled: bool) -> None:
    vpc = get_vpc_path()
    if vpc and Path(f"{vpc}/instant_boot").exists():
        Path(f"{vpc}/instant_boot").write_text("1" if enabled else "0")
